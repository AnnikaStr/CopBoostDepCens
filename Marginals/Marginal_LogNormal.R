...
LogNormal_Mar <- function(loc = NULL, offset_sigma = NULL, dep = FALSE){
  ...
  delta <- parse_expr(paste("y[,", 2, "]", sep = ""))
  
  # dependent censoring?
  if (dep){
    y <- parse_expr(paste("y[,", 1, "]", sep = ""))
    delta <- parse_expr(paste("y[,", 2, "]", sep = ""))
  }                        
  ...
  pdf_log_sig <- expr(dnorm(x = !!y, meanlog = !!mu, sdlog = exp(f),log = T))
  # derivative pdf
  derlpdf1.deretasig  <- expr((1/(exp(f)^3)) * ((log(!!y) - !!mu)^2 - exp(f)^2) * exp(f))
  # cdf 
  cdf_sig <- expr(plnorm(q = !!y, meanlog = !!mu, sdlog = exp(f)))
  cdf_log_sig <- expr(pnorm(q = !!y, meanlog = !!mu, sdlog = exp(f),log.p = T))
  # derivative cdf 
