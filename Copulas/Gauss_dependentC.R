...
source("Marginals/Marginal_Gaussian.R")
source("Marginals/Marginal_Gamma.R")
source("Marginals/Marginal_Weibull.R")
### Gauss-Copula function

Gauss_Cop_depCens <- function(marg1 = NULL, marg2 = NULL,
  ...
  gradient_marg1_gen <- function(derlpdf1, cdf1, cdf2, dercdf1){
    expr({
      ...
      h1 <- pdffz(pnorm( (qnorm(F2)- (rho)*qnorm(F1))/(sqrt(1-rho^2)) ))
      h2 <- pdffz(pnorm( (qnorm(F1)- (rho)*qnorm(F2))/(sqrt(1-rho^2)) ))
        
      
      x_nachu1 <-sqrt(2*pi)/(exp(-(qnorm(F1)^2)/2))
      h1_deru1 <- (dnorm( (qnorm(F2)- (rho)*qnorm(F1))/(sqrt(1-rho^2)))  *(-rho/(sqrt(1-rho^2)))*x_nachu1)

      h2_deru1 <- (dnorm( (qnorm(F1)- (rho)*qnorm(F2))/(sqrt(1-rho^2))) *(1/(sqrt(1-rho^2)))*(sqrt(2*pi)/exp(-(qnorm(F1)^2)/2)))
  ...
  gradient_marg2_gen <- function(derlpdf2, cdf1, cdf2, dercdf2){
   expr({
     ...
     x_nachu2<-sqrt(2*pi)/(exp(-(qnorm(F2)^2)/2))
     h2_deru2 <- (dnorm( (qnorm(F1)- (rho)*qnorm(F2))/(sqrt(1-rho^2)))  *(-rho/(sqrt(1-rho^2)))*x_nachu2)
     
     h1_deru2 <- (dnorm((qnorm(F2) - rho*qnorm(F1))/sqrt(1 - rho^2)) * (1/(sqrt(1-rho^2))) * (sqrt(2*pi)/exp(-(qnorm(F2)^2)/2)))
       
    ngr <- (
         y[,2] *  (1/pdffz(1 - h1) * (-1)* h1_deru2 *df2) +
           (1 - y[,2]) * ( dpdf2 + (1/pdffz(1 - h2) * h2_deru2 * (-1)*df2))
       )
   })
  }
  ...
  gradient_cop_gen <- function(cdf1, cdf2, rho){
    expr({
      ...
      h1 <- (pnorm( (qnorm(F2)- (!!rho)*qnorm(F1))/(sqrt(1-(!!rho)^2)) ))
      h2 <- (pnorm( (qnorm(F1)- (!!rho)*qnorm(F2))/(sqrt(1-(!!rho)^2)) ))
      
      h2_derCop <-  (dnorm( (qnorm(F1)- (!!rho)*qnorm(F2))/(sqrt(1-(!!rho)^2)))* ( qnorm(F1)*(!!rho) - qnorm(F2))/(1-(!!rho)^2)^(3/2)) 
      h1_derCop <-  (dnorm( (qnorm(F2)- (!!rho)*qnorm(F1))/(sqrt(1-(!!rho)^2)))* ( qnorm(F2)*(!!rho) - qnorm(F1))/(1-(!!rho)^2)^(3/2))
        
        ngr <- ( 
          y[,2] * ( 1/pdffz(1 - h1) * h1_derCop * (-1) *(1/cosh(f)^2))  + 
            (1 - y[,2]) * ( 1/pdffz(1 - h2) * h2_derCop * (-1) *(1/cosh(f)^2))
        )
